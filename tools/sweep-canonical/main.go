...
func sweepCLI(cliDir, ownerName string, readmeOnly, attributionOnly bool, contributors []person) (sweepStatus, error) {
	...
	// agentcookie.toml emit. Skipped CLIs (cookie-only, manual-override
	// marker) silently return changed=false; the render is queued as a
	// regular edit so the shared write-with-rollback loop covers partial
	// write failures (e.g. ENOSPC mid-write) like every other surface.
	...
	if agentcookieChanged {
		...
		edits = append(edits, fileEdit{agentcookiePath, agentcookieBefore, []byte(agentcookieBody)})
	}
